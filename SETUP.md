...
### Публикация Gradio “наружу” (backend остаётся локальным)

В вашем проекте Gradio вызывает backend **с серверной стороны** (из python-процесса) по `API_BASE_URL` (по умолчанию `http://127.0.0.1:8000`). Поэтому можно сделать публичной только Gradio-ссылку, а backend не выставлять напрямую.

1) Запустите backend локально (как обычно).

2) Запустите Gradio с включённым share и (желательно) паролем:

```powershell
cd c:\ai-assistant

$env:GRADIO_SHARE="true"
$env:GRADIO_AUTH="demo:demo-pass"   # замените на свои
$env:API_BASE_URL="http://127.0.0.1:8000"

python demo/gradio_app.py
```

Gradio выведет **публичный URL**, доступный из интернета, пока у вас запущен процесс `python demo/gradio_app.py`.

Примечания:
- **Без `GRADIO_AUTH` ссылку сможет открыть любой, у кого она есть.**
- Если вы хотите доступ в локальной сети без туннеля: `GRADIO_SERVER_NAME=0.0.0.0` (и открывайте порт на фаерволе).
