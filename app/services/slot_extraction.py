...
# ============================================================================
# Продуктовые слоты и дополнительные сущности
# ============================================================================
...
PRODUCT_STOPWORDS = [
    ...
    r"\bв корзин[аеы]?\b",
    r"\bиз корзин[аеы]?\b",
    r"\bв избранн(ое|ое|ый)?\b",
    r"\bиз избранн(ого)?\b",
    r"\bаналоги?\b",
    r"\bаналог\b",
    r"\bинструкц(ия|ии)?\b",
    r"\bпротивопоказан\w*\b",
    r"\bналичи[ея]\b",
    r"\bгде купить\b",
    r"\bесть ли\b",
    r"\bподробнее о\b",
...
def extract_product_phrase(message: str, normalized: str | None = None, intent: str | None = None) -> Optional[str]:
    """
    Извлекает название товара, убирая служебные слова и действия.
    """
    text = message or ""
    if not text.strip():
        return None
    normalized_text = normalized or normalize_text(text)
    cleaned = text
    for pattern in PRODUCT_STOPWORDS:
        cleaned = re.sub(pattern, " ", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\s+", " ", cleaned).strip(" ,.:;!?")
    # Если после чистки ничего не осталось, пробуем normalized
    if not cleaned:
...
def extract_quantity(message: str) -> int:
    """Извлекает количество товара. Если не найдено — возвращает 1."""
    normalized = normalize_text(message)
    digit_match = re.search(r"\b(\d+)\s*(?:шт|штук|упаков|упак)?", normalized)
    if digit_match:
        try:
...
def extract_promo_code(message: str) -> Optional[str]:
    """Извлекает промокод в виде последовательности букв/цифр/подчёркиваний."""
    if not message:
        return None
    upper = message.upper()
    promo_match = re.search(r"(?:ПРОМОКОД\s+)?([A-Z0-9_-]{4,})", upper)
    if promo_match:
...
def extract_category(message: str) -> Optional[str]:
    """Определяет категорию из сообщения по словарю категорий."""
    normalized = normalize_text(message)
...
def extract_inn(message: str) -> Optional[str]:
    """Извлекает МНН/ИНН из запроса."""
    normalized = normalize_text(message)
    for value in INN_VALUES:
        if value in normalized:
            return value
    for marker in INN_KEYWORDS:
