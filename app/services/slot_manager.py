...
DEFAULT_SLOT_QUESTIONS: Dict[str, str] = {
    ...
    "product_name": "Какой товар имеете в виду? Укажите название.",
    "product_id": "Какой товар имеете в виду? Можно просто написать его название.",
...
class SlotManager:
    ...
    def _get_slot_prompt(
        ...
        # Если ожидается product_id, но есть вопрос для product_name — используем его (новый контракт слотов).
        if slot_name == "product_id" and "product_name" in slot_questions:
            return slot_questions["product_name"]
        if slot_name == "product_id" and "product_name" in self._default_slot_questions:
            return self._default_slot_questions["product_name"]
    ...
    def _question_for_slot(self, intent: IntentType | None, slot: str) -> str:
        ...
        if intent and intent in self._intent_slot_questions:
            ...
            if question:
                return question
            # Если слот product_id, а в конфиге есть product_name — используем его
            if slot == "product_id":
                name_q = self._intent_slot_questions[intent].get("product_name")
                if name_q:
                    return name_q
