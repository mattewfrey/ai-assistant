...
### 6) Обязательные citations (used_fields)
Ты ОБЯЗАН вернуть used_fields: список путей к полям в context_json, на которых основан ответ.
- Любое фактическое утверждение должно иметь хотя бы 1 поле в used_fields.
- Если данных нет → used_fields=[].
- Не выдумывай пути.
